...
export const initializeTraefik = async ({
  ...
  try {
    // 添加更多调试日志
    console.log("Traefik Settings:", JSON.stringify(settings, null, 2));
    if (serverId) {
    ...
    // 打印服务检查结果
    console.log("Traefik Service Inspect:", JSON.stringify(inspect, null, 2));
    ...
    console.error("Traefik Error:", error);
...
export const getDefaultServerTraefikConfig = () => {
  const configObject: MainTraefikConfig = {
    providers: {
      docker: {
        defaultRule: "Host(`{{ trimPrefix `/` .Name }}.traefik.me`)",
